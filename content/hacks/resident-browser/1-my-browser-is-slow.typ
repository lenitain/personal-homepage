...
先看一个最小的例子。两个程序都只打印一行字，一个静态链接（自足，第 3 节那条路），
一个动态链接（要用解释器，第 4、5 节那条路）：
...
回忆第 1 节：`fork` 复制出来的子进程，拿到的是一份已经建好的地址空间的副本
（内核用写时复制，所以并不真的拷一份内存出来）。也就是说，库的映射、重定位的结果、
初始化跑完的状态，*全都在里面了*。第 4、5 节那四件事，一件都不用再做。

两条路的差别可以直接量：

#lab("演示 04：fork 继承 vs exec 重来")[
  造一个「很贵」的库（初始化时要准备 8 MB 数据）。两条路各派生 40 次：
...
$ （列出所有 QtWebEngine 进程：pid / 父进程 / 类型）
  1633    1357    zygote
  1634    1357    zygote
  1636    1634    zygote      ← 它的父进程是另一个 zygote
  1689    1636    renderer    ← 它的父进程是 zygote
  6151    1636    renderer
  112935  1636    renderer
```

浏览器本体（1357）先启动两个「模板进程」；其中一个再派生出一个渲染器模板；
*每一个网页渲染器都是从那个模板 fork 出来的*。所以打开一个新标签页，
不需要把上面那一整套重新走一遍。
...
+   如果贵的那部分能跨多次使用共享，就用 `fork` 继承，不要 `exec` 重来

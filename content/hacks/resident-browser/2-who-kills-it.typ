...
#ask[
  ...
  问题不在于它不管用 —— 它管用。一个错的工具如果当场就报错，你会立刻换一个；
  它要是能用，你就会一直在它上面打补丁。
]

= 3. 非特权用户拿不到 PID namespace
...
这里有一点容易漏掉：*user namespace 的默认状态是「你没有身份」，
...
#note[
  `--map-current-user` 这个选项本身就说明问题：一个*隔离*工具
  默认不给你身份，你得额外请求「顺便把我也映进去」。它的作者没打算让你在这儿过日子。
]

= 4. 代价一：user namespace 改掉了你的 uid
...
这里要问的是：同一个 namespace 里，为什么 D-Bus 断了而别的都没断？

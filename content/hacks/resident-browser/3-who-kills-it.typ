...
开着标签页的实例是五个往上；就算零窗口，帮手也不会少
（#chapterRef("3.") 的最后一节量了这件事）。
所以你欠的不是「关掉一个进程」，是*释放一整棵树的资源*。

这件事平时完全看不出来。开发的时候反复启动、反复关窗，一切正常；
只有真的注销之后，`ps` 里才会出现一堆没有爹的 renderer —— 而那时候你已经在做别的事了。

= 1. 先确认一件事：父进程死了，子进程不会跟着死

这一章要解决的问题是「谁来释放这棵树」，而它的前提是：*这棵树真的会比启动它的东西活得久*。
这件事在使用者的经验里是反的（「关掉终端程序就没了」），所以先量一下。

#lab("演示 15：父进程退出之后，那个子进程归谁")[
  ```sh
  $ ./15-orphan-reaping.sh
    子 shell 起了个 sleep（pid …）就退出了：
      sleep 状态 S（还活着），现在的父进程是 pid 1095（systemd）
    它没有死 —— 内核把它交给了*离它最近的 subreaper*：
    你的会话里通常就是 user manager（systemd --user），否则是 pid 1。
  ```

  内核做的只有一件事：原来那个父进程没了，就从这个进程往上找，
  找到第一个 subreaper，换它当爹。*一步都不需要你安排。*

  但它不做的也很清楚：接管 ≠ 收拾。那个进程活着时占的东西、留下的文件、
  开着的端口，没有任何一处会因为「它被 pid 1095 接管了」而被清理。

  （完整脚本：`./docs/labs/resident-browser/15-orphan-reaping.sh`）
]

所以正确的说法不是「常驻进程怎么让它活着」，而是：
*它会活着，这是内核保证的；问题是它的死该由谁负责。*

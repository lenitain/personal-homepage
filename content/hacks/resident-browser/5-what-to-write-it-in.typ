...
#chapterRef("3.") 把常驻的部分和它的代价都说完了。剩下那条*热路径*：
按下快捷键之后、到消息送达之前的那几百微秒。这一段归启动器管，
于是要做一个决定：*用它什么写。*

听起来这是在选语言 —— 挑一门快的。可这条路径上的程序只做三件事：
找一个 socket，写大约一百个字节，退出。工作量小到可以忽略，
于是这个实验测出来的其实是另一件事：*在某门语言里「成为一个进程」要花多少代价。*
这个量平时观测不到，因为它总被程序真正在干的事情盖住 ——
只有把程序本身剥到接近零，它才露出来。

所以比的不是「哪门语言好」，而是每门语言在你的代码跑起来之前做了什么。
...
#lab("演示：execve 成功那一刻的地址空间，和一整轮的系统调用（measure.sh）")[
  ...
  `vma` 是 `execve` 成功那一刻，地址空间里有多少个区间（VMA，虚拟内存区域），
  `rss` 是那一刻的常驻内存（KB）。

  #note[
    这一节的 `p50` 比第 1 节那张延迟表大了一倍多，别当成两个结论 ——
    两支脚本量的不是一回事：`latency.sh` 绑核、报最小值，`measure.sh` 不绑核、报 p50。
    同一个二进制在两种口径下差多少，看 `qb-open-c-musl-static` 就够了：
    第 1 节 *377.6*，这里 *543.4*。延迟结论只看第 1 节，这一节看的是系统调用和地址空间。
  ]
...
Zig 落在 *9 个 VMA（区间）、12 KB RSS*，跟手写汇编*完全一样*，系统调用 15 对 10。
...
#note[
  这张表来自它自己那一轮测量，跟第 1 节那张延迟表里的 22.7 ms 几乎一样，
...
#oops(label: "计划外的收获")[
  这一段不是设计出来的：写第二份实现的时候，撞出一个已经发布、每天都在用的内存越界写。

  它能露出来，只是因为同一个行为被写了两遍 —— 每个移植版本都从头做了边界检查，
  C 版那处没做的地方才有人质疑。

  这张对比表是这一章要讲的东西；那个 bug 是它顺手给我的东西。
]

= 8. 回到入口

四个决定到这里各做了一次。它们合起来那句话，写在入口那一页的结尾。

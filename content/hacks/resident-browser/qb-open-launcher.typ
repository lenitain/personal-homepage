...
启动器是这个系列里最无聊的程序。它找一个 socket，写大约一百个字节，然后退出。
没有任何算法可言，也没有任何可优化的地方。

*这正是它值得测量的原因。* 把程序本身剥掉之后，剩下的就是
*在某门语言里「成为一个进程」的代价* —— 一个平时根本没法单独观测的量，
因为它总被程序真正在干的事情埋住。

所以我把同一个启动器写了九遍，从四个角度量它。

= 方法

- *正确性给基准把关。* 每一个实现都必须跟已发布的 C 版吐出*逐字节相同*的 IPC 消息，
  覆盖八个用例：两个参数、无参数、空参数、引号与反斜杠、换行与制表符、C0 控制字符、
  裸 UTF-8、2000 字节的长参数。九个实现全部通过。一个因为发错字节而变快的启动器，
  不是启动器。
- *延迟*是 `fork` → `execve` → 干活 → 被回收，用一个 C 写的小工具拿
  `clock_gettime` 夹住 `wait4` 来计时。三轮 × 500 次，绑在同一个核上，
  报三轮*最小值的中位数*。最小值是受干扰最少的那一次 —— 在一台开着桌面会话的机器上，
  它最接近真实开销。
- *一个「底噪」二进制。* 一个静态二进制，函数体只有 `exit_group`。它什么都不干，
  所以它测出来的就是*每一个*实现都要付的「被启动」的钱。没有它，
  「300 微秒」这个数没法解释。
- *系统调用*来自 `strace`；*地址空间*来自 `execve` 成功那一刻的 `ptrace` 停点 ——
...
而在这个尺度上，它相对「存在的代价」是个舍入误差。那条线以上的部分，
全是语言的运行时在你的代码跑起来之前做的事。

所以，为一个热路径启动器选语言，*不是在选语法或者表达力，而是在选「什么东西会跟着你一起启动」。*
...
三比零。动态版本要为 `ld.so` 的启动、符号解析、映射库付钱 —— 这在 glibc 的 syscall 记录里
看得一清二楚：8 次 `mmap`，加一堆对着库缓存的 `openat` / `fstat`。

但静态不是免费的，它是*预付*的：

- glibc 静态是 *826 KB 对 15 KB* —— 56 倍的字节，换 118 µs。
- musl 静态是 *46 KB*。*这才是这笔交易真正划算的那个版本。*
...
也就是说，*大约 3% 的运行时间是程序被写出来要做的那件事。* 剩下 97% 是可以避免的 ——
...
#think[
  现在回头看总表：如果你只需要「快」，汇编比 C 快 29 µs、小 37 KB。

  这 29 µs 值 400 行手写汇编吗？先自己回答，再看我最后怎么选的。
]
...
- *汇编* —— 10 个系统调用，而且正好就是那件工作：`openat`、`getdents64`、`socket`、
...
这一段是整个系列里我最想留下的部分，因为它不是设计出来的，是*撞出来的*。
...
#lab("实验 10：亲眼看一下这个边界")[
  ...
  这个脚本*必须*在私有 mount namespace 里跑，并把 `/bin/true` 绑到
  `/usr/bin/qutebrowser` 上 —— 因为如果越界写出来的字节恰好构成一个合法的指针数组，
  `execve` 是*能成功的*，那就真的会拉起用户的浏览器。这个安全网不是谨慎，
  是这个 bug 的失败模式决定的。
...
每一个其它语言的移植版本从一开始就做了边界检查 —— 这是这个差异唯一浮出水面的原因。
修法是加一个小 `literal_append` 辅助函数，跟转义函数共用一个容量上限，
于是没有任何一次追加能把偏移量推出末尾。

#wrong[
  我本来以为「用五门语言写同一个十行程序」是一次纯粹的教学演示，
  它的价值在于那张对比表。

  实际价值最高的是它顺手抓出来的那个 bug —— 一个已经发布、每天都在用的
  内存越界写。*多个独立实现是很好的测试套件*，这一点我原来没预料到。
]

...
#slide[
  ...
  剩下那条*热路径*：你按下快捷键之后、到消息送达之间，那几百微秒是怎么花掉的。

  #note[完整版见左边文件树里的 `5-what-to-write-it-in.typ`。]
]
...
#slide[
  ...
  每一个实现都必须跟已发布的 C 版吐出*逐字节相同*的 IPC 消息，
...
#slide[
  ...
  *9 个 VMA、12 KB RSS*，跟手写汇编完全一样，系统调用 15 对 10。
...
#slide[
  ...
  其中六个 `rt_sigaction`、五个 `brk`。

  那是 `std` 在你的 `main` 之前搭运行时 —— 而这里 `main` 做的事，
  是往一个 socket 写一百个字节。
...
#slide[
  = C 版有一个已经发布、每天都在用的越界写

  C 版用 `snprintf` 拼 JSON，然后把返回值加进写入偏移量 ——
  而它返回的是*本应写入*的长度。

  截断 → 偏移量冲过 8192 字节缓冲区的末尾 → 下一次写入落到 `.bss` 里
  紧挨着 `environ` 的地方。

  触发条件是一个大约 8 KB 的参数，而这段代码已经发布、每天都在用。
]

#slide[
  = 越界之后它不崩溃、不报错，而是换了个行为

  快路径发不出去，于是走 fallback，冷启动一整个真浏览器。
  用户看到的现象是「怎么突然变慢了」。

  每个其它语言的移植版从一开始就做了边界检查 ——
  这是它唯一浮出水面的原因。

  #note[现场跑：`./docs/labs/resident-browser/overflow-check.sh`]
]

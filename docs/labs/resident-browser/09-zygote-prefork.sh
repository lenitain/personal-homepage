...
#
# 承接 08 —— 找库、映射、重定位、调 init 这四步，跟你的程序做了什么毫无关系，
# 每次启动都要付。这一份看能不能不付第二次。
#
...
cat <<EOF
  ...
  B 路比 A 路慢 $ratio 倍；多出来的 $(( b_init - a_init )) 遍构造函数，就是 08 里那四步：找库、映射、重定位、
  调 init —— 只跟「这份程序映像」有关，跟这次任务要干什么毫无关系。fork 不 exec，子进程拿到的是
...
cat <<'EOF'
  ...
  常驻的东西总得有人负责它的死（10 试的就是 PID namespace 这条路）。
EOF

...
| 3 | 光标移动是否换预览 | **不换**。光标只是光标，Enter / 单击才打开 —— ⛔ **2026-09-24 推翻**：改为「选中即所见」，见[键盘交互改版 spec](./2026-09-24-intuitive-tree-keys-design.md) |
| 4 | 键盘范围 | 只保留 ↑ / ↓ / Enter。Esc、←、→、Backspace 全部移除 —— ⛔ **2026-09-24 推翻**：←/→ 及 Home/End/PageUp/PageDn/首字母跳转回归，见[键盘交互改版 spec](./2026-09-24-intuitive-tree-keys-design.md) |
...
| 7b | 开合动画 | 左栏开合走 160ms 的 `slide`（x 轴）。内层定宽 280px，所以动画是「从窄边条那里展开 / 卷回去」，不会把树里的文件名一路挤扁。系统开了 `prefers-reduced-motion` 时时长归零 |
...
| 点窄边条 | 切换左栏开合（160ms 过渡）。它永远在 `x = 0`、宽度不变，只有字形在 `«` / `»` 之间切 |
...
- **竖分割线只有一条，长在 `.pane` 的 `border-left` 上**：它永远贴着正文左边缘，开合动画期间跟着正文一起移动。窄边条和左栏都不画右边框 —— 否则收起时窄边条的线和正文的线会并成 2px

  > 踩过的坑：试过把分割线写成 `.sidebar` 的 `inset box-shadow`。box-shadow（含 inset）画在子元素背景**之下**，而内层 `.file-tree` 有铺满的 `--bg0` 背景，直接把它盖没了。另外 `border` 在 `box-sizing: border-box` 下本来就贴着右边缘、会跟着宽度动画走，当初以为它不会动是判断错了。

...
| 6 | 左栏整体收起 | 收起后左边缘留 **22px 窄边条**，上面一个 `»`；展开时 `«` 在树面板右上角 |
...
| 点 `«` | `sidebarOpen = false`，`expanded` 原样保留 |
| 点 `»` | `sidebarOpen = true`，树恢复到收起前的样子 |
...
左栏的 `«` 按钮和收起后的窄边条直接写在 `+page.svelte` 里，不单开组件：它们只是两个按钮，没有独立状态。
